...
mycrypto() {
  ...
  echo -n "\"full_text\":\" $(~/.config/i3/status/crypto.py) \","
  echo -n "\"color\":\"#000000\","
...
myip_public() {
  ...
  echo -n "\"full_text\":\" $(~/.config/i3/status/ip.py) \","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
}
...
disk_usage() {
  ...
  echo -n "\"full_text\":\"  $(~/.config/i3/status/disk.py)%\","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "}"
}
...
memory() {
  ...
  echo -n "\"full_text\":\"  $(~/.config/i3/status/memory.py)%\","
...
cpu_usage() {
  ...
  echo -n "\"full_text\":\"  $(~/.config/i3/status/cpu.py)% \","
  echo -n "\"background\":\"#3949AB\","
  common
  echo -n "},"
}
...
meteo() {
  ...
  echo -n "\"full_text\":\" $(~/.config/i3/status/meteo.py) \","
  echo -n "\"background\":\"$bg\","
  common
  echo -n "},"
}
...
volume() {
  local bg="#673AB7"
  separator $bg $bg_separator_previous  
  vol=$(amixer get Master | egrep -o '[0-9]{1,3}%')
  echo -n ",{"
...
while read line;
do
  # echo $line > /home/aucarle/tmp.txt
  # {"name":"id_vpn","button":1,"modifiers":["Mod2"],"x":2982,"y":9,"relative_x":67,"relative_y":9,"width":95,"height":22}

  # VPN click
  if [[ $line == *"name"*"id_vpn"* ]]; then
    xfce4-terminal -e ~/.config/i3/status/click_vpn.sh &

  # CHECK UPDATES
  elif [[ $line == *"name"*"id_systemupdate"* ]]; then
    echo 'bite' > /home/aucarle/tmp.txt
    alacritty -e ~/.config/i3/status/click_checkupdates.sh &
  ...
  # TIME
    ...
    xfce4-terminal -e ~/.config/i3/status/click_time.sh &
